...
def upload_video(youtube, video_path, title, description, tags, privacy="public", category="17", publish_at=None):
    body = {
        "snippet": {
            "title": title,
    ...
    vid = response["id"]
    if publish_at:
        print(f"  Scheduled! https://youtu.be/{vid} (goes public at {publish_at})")
    else:
...
def main():
    ...
    parser.add_argument("--publish-at", help="ISO8601 datetime for scheduled publish (e.g. 2026-06-12T00:00:00Z)")
    ...
    if args.video and args.metadata:
        ...
        upload_video(youtube, args.video, title, description, tags, args.privacy, publish_at=args.publish_at)
        return
    ...
    print("  Schedule premiere: python scripts/upload_youtube.py --video <file> --metadata <json> --publish-at 2026-06-12T00:00:00Z")
